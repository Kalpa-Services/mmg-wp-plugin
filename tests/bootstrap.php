...
spl_autoload_register(
	function ( $class_name ) {
		if ( strpos( $class_name, 'MMG\\CheckoutPayment\\' ) === 0 ) {
			$file = dirname( __DIR__ ) . '/includes/class-' . strtolower( str_replace( array( 'MMG\\CheckoutPayment\\', '\\', '_' ), array( '', '-', '-' ), $class_name ) ) . '.php';
			if ( file_exists( $file ) ) {
				require_once $file;
			} else {
				die( esc_html( 'Class file not found for ' . $class_name . ' at ' . $file ) );
			}
		}
	}
);
...
function _init_mmg_checkout_payment() {
	new \MMG\CheckoutPayment\MMG_Checkout_Payment();
}
