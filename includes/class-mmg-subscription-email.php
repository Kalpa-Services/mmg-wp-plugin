...
<p>— {site_name}</p>',
	...
	public function send_reminder( object $sub, string $payment_url ): void {
		$this->send( 'mmg_email_tpl_reminder', $sub, array( '{payment_url}' => $payment_url ) );
	}
	 ...
	 */
	private function send( string $option_key, object $sub, array $extra_vars = array() ): void {
		$tpl = get_option( $option_key, array() );
		if ( empty( $tpl['subject'] ) || empty( $tpl['body'] ) ) {
			$tpl = self::DEFAULT_TEMPLATES[ $option_key ] ?? array(
				'subject' => '',
				'body'    => '',
			);
		}
		...
		wp_mail( $email_to, $subject, $body, array( 'Content-Type: text/html; charset=UTF-8' ) );
	}
